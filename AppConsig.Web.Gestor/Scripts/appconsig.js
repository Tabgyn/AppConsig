...
// Função para usar mascara de campo decimal.
(function ($) {
    "use strict";
    if (!$.browser) {
        $.browser = {};
    ...
    var methods = {
        ...
        init: function (settings) {
            ...
            return this.each(function () {
                ...
                $input.unbind(".decimalMask");
                $input.bind("keypress.decimalMask", keypressEvent);
                $input.bind("keydown.decimalMask", keydownEvent);
                $input.bind("blur.decimalMask", blurEvent);
                $input.bind("focus.decimalMask", focusEvent);
                $input.bind("click.decimalMask", clickEvent);
                $input.bind("cut.decimalMask", cutPasteEvent);
                $input.bind("paste.decimalMask", cutPasteEvent);
                $input.bind("mask.decimalMask", mask);
            });
        }
    };

    $.fn.decimalMask = function (method) {
        if (methods[method]) {
            return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === "object" || !method) {
            return methods.init.apply(this, arguments);
        } else {
            $.error("Method " + method + " does not exist on jQuery.decimalMask");
        }
        return false;
    };
